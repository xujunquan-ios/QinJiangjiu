...
- (void)loadView
{
    ...
    _navigationbar                      = [[UIToolbar alloc] initWithFrame:CGRectZero];
	...
	// create buttons for navigationbar
    _leftBarButton = [[UIBarButtonItem alloc] initWithTitle:@"Back" style:UIBarButtonItemStylePlain target:self action:@selector(back)];
    _rightBarButton = [[UIBarButtonItem alloc] initWithTitle:@"See All" style:UIBarButtonItemStylePlain target:self action:@selector(handleSeeAllTouch:)];
	
    NSMutableArray *navigationItems = [[NSMutableArray alloc] init];
	[navigationItems addObject:_leftBarButton];
	[navigationItems addObject:_rightBarButton];
		
	// set buttons on the toolbar.
	[_navigationbar setItems:navigationItems animated:NO];
//    Etrelease(navigationItems);
    // build stuff
    [self reloadGallery];
}
...
- (void)destroyViews {
    ...
    for (NSInteger i=0; i<[photoKeys count]; i++) {
...
{
    return [_photoLoaders objectForKey:[NSString stringWithFormat:@"%ld", (long)_currentIndex]];
}
...
- (void)removeImageAtIndex:(NSUInteger)index
{
    ...
    if (_photoThumbnailViews.count > index){
...
- (void)positionInnerContainer
{
	...
	{//portrait
		innerContainerRect = CGRectMake( 0, 0 - [UIApplication sharedApplication].statusBarFrame.size.height, _container.frame.size.width, screenFrame.size.height );
	}
	else 
	{// landscape
		innerContainerRect = CGRectMake( 0, 0 - [UIApplication sharedApplication].statusBarFrame.size.height, _container.frame.size.width, screenFrame.size.width );
	}
...
- (void)positionNavigationbar
{
	_navigationbar.frame = CGRectMake( 0, 0, _scroller.frame.size.width, kToolbarHeight );
}

- (void)resizeThumbView
{
    NSInteger barHeight = kToolbarHeight;
...
//    }
	_thumbsView.frame = CGRectMake( 0, barHeight, _innerContainer.frame.size.width, _innerContainer.frame.size.height-barHeight );
}
...
- (void)updateScrollSize
{
	CGFloat contentWidth = _scroller.frame.size.width * [_photoSource numberOfPhotosForPhotoGallery:self];
...
- (void)updateTitle
{
	[self setTitle:[NSString stringWithFormat:@"%ld of %ld", (long)(_currentIndex+1), (long)[_photoSource numberOfPhotosForPhotoGallery:self]]];
}
...
- (void)moveScrollerToCurrentIndexWithAnimation:(BOOL)animation
{
	NSInteger xp = _scroller.frame.size.width * _currentIndex;
